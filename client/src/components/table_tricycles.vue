...
export default {
	...
	mounted: function() {
		Axios.get("/get_tricycles_list/" + this.is_admin)
			.then(res => {
				const data = res.data;
	...
	methods: {
		...
		on_delete: function() {
			...
			{
				Axios.post("/delete_tricycle", {
					tricycle_id: this.selected_item.id
				})
				.then(res => {
					const data = res.data;
					if (data.success) {
	...
	data: function() {
		return {
			...
			fields: [
				...
				{key: "plate_number", sortable: false, visible: true, label: "Plate Number"},
